    ...
    if (context.projectType === ProjectType.BI_BALLERINA) {
        ...
        if (vscode.workspace.workspaceFolders?.length && context.isInWi) {
            ...
            if (selectedProfile !== MI_PROFILE) {
                vscode.window.showWarningMessage('We couldn\'t detect the project type. Please ensure you have a valid WSO2 Ballerina, Streaming Integrator project open.', { modal: true }, 'Go to Welcome Screen', 'Open another folder').then(selection => {
