...
export class Git {
	...
	async isEmptyRepository(repositoryPath: string): Promise<boolean> {
		try {
			...
			ext.logError("Error while checking if repository is empty");
			ext.logError(String(error));
		}
		return false;
	}
	...
	static parse(raw: string): GitConfigSection[] {
		...
		for (const line of raw.split(GitConfigParser._lineSeparator)) {
			...
			if (sectionMatch?.length === 3) {
				addSection(section);
				section = { name: sectionMatch[1], subSectionName: sectionMatch[2]?.replace(/"/g, ""), properties: {} };
